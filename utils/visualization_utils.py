...
def draw_bounding_box_on_image_array(current_frame_number, image,
  ...
  """
    image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
    is_vehicle_detected, csv_line, update_csv = draw_bounding_box_on_image(current_frame_number,
                                                                           image_pil, ymin, xmin,
                                                                           ymax, xmax, color,
                                                                           thickness,
                                                                           display_str_list,
...
def draw_bounding_box_on_image(current_frame_number, image,
  ...
  """
    ...
    if (x_axis[0] == 1):
        predicted_direction, is_vehicle_detected, update_csv = object_counter_x_axis.count_objects_x_axis(
            top, bottom, right, left, detected_vehicle_image, ROI_POSITION[0],
            ROI_POSITION[0] + DEVIATION[0], ROI_POSITION[0] + (DEVIATION[0] * 2), DEVIATION[0])
    elif (mode_number[0] == 2):
        predicted_direction, is_vehicle_detected, update_csv = object_counter.count_objects(top,
                                                                                            bottom,
                                                                                            right,
                                                                                            left,
...
def visualize_boxes_and_labels_on_image_array(current_frame_number,
  ...
  """
    ...
    if (mode == 1):
        ...
        counting_mode = ''.join(
            [i for i in counting_mode.replace("['", " ").replace("']", " ").replace("%", "") if
             not i.isdigit()])
...
def vis_box_and_label_flex_axis(current_frame_number,
                                image,
                                mode,
                                color_recognition_status,
                                boxes,
    ...
    csv_line_util = "not_available"
    counter = 0
    ROI_POSITION.insert(0, reference)
    DEVIATION.insert(0, deviation)
    x_axis.insert(0, 1)
    is_vehicle_detected = []
    mode_number.insert(0, mode)
